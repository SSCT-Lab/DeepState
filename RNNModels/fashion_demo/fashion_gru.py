...
from keras.layers import Input, Lambda, GRU, Dense
...
# loss: 0.1944 - accuracy: 0.9274 - val_loss: 0.3235 - val_accuracy: 0.8921
...
class FashionGRUClassifier:
    ...
    def create_model(self):
        ...
        self.model.add(GRU(self.n_units, input_shape=(self.time_steps, self.n_inputs), name="gru"))
    ...
    def load_hidden_state_model(self, model_path):
        ...
        gru = GRU(self.n_units, input_shape=(self.time_steps, self.n_inputs), return_sequences=True, name="gru")(input)
        last_timestep = Lambda(lambda x: x[:, -1, :])(gru)
        dense = Dense(10, activation='softmax', name="dense")(last_timestep)
        model = Model(inputs=input, outputs=[dense, gru])
    ...
    def reload_dense(self, model_path):
        ...
        dense = Dense(10, activation='softmax', name="dense")(input)
...
if __name__ == "__main__":
    (x_train, y_train), (x_test, y_test) = fashion_mnist.load_data()
    save_path = "./RNNModels/fashion_demo/models"
